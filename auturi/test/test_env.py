...
def _test_env(test_env, num_envs, num_steps, timeout):
    ...
    if not isinstance(test_env, AuturiSerialEnv):
        test_env.start_loop()
...
def create_vector_env(num_envs, num_parallel):
    test_env_fn, _, _ = utils.get_create_fn(num_envs, backend=BACKEND)
    test_env = test_env_fn()
    mock_reconfigure(test_env, num_envs, num_parallel)
    return test_env
...
def test_vector_env_basic():
    vector_env = create_vector_env(1, 1)
    assert vector_env.num_envs == 1
    assert vector_env.num_workers == 1
    _test_env(vector_env, num_envs=1, num_steps=3, timeout=0.5 * 3)
    vector_env.terminate()


def test_fully_parallel():
    vector_env = create_vector_env(3, 3)
    assert vector_env.num_envs == 3
    assert vector_env.num_workers == 3
    agg_obs = _test_env(vector_env, num_envs=3, num_steps=3, timeout=0.5 * 3)
...
def test_serial():
    vector_env = create_vector_env(2, 1)
    assert vector_env.num_envs == 2
    assert vector_env.num_workers == 1

    agg_obs = _test_env(vector_env, num_envs=2, num_steps=3, timeout=(0.5 + 0.5) * 3)
    print(agg_obs)
...
def test_combination():
    vector_env = create_vector_env(4, 2)
    assert vector_env.num_envs == 4
    assert vector_env.num_workers == 2
    agg_obs = _test_env(vector_env, num_envs=4, num_steps=3, timeout=(0.5 + 0.5) * 3)
...
def test_reconfigure():
    ...
    assert vector_env.num_envs == 1
    assert vector_env.num_workers == 1
    agg_obs1 = _test_env(vector_env, num_envs=1, num_steps=3, timeout=0.5 * 3)
    assert np.all(agg_obs1 == np.array([1001, 1002, 1003]))

    mock_reconfigure(vector_env, num_envs=2, num_parallel=2)
    assert vector_env.num_envs == 2
    assert vector_env.num_workers == 2
    agg_obs2 = _test_env(vector_env, num_envs=2, num_steps=3, timeout=0.5 * 3)

    mock_reconfigure(vector_env, num_envs=2, num_parallel=1)
    assert vector_env.num_envs == 2
    assert vector_env.num_workers == 1
    agg_obs3 = _test_env(vector_env, num_envs=2, num_steps=3, timeout=(0.5 + 0.5) * 3)
...
def test_rollouts():
    vector_env = create_vector_env(4, 1)
    ...
    agg_obs = _test_env(vector_env, num_envs=4, num_steps=2, timeout=(0.5 + 0.5) * 2)
    assert len(agg_obs) == 8

    mock_reconfigure(vector_env, num_envs=4, num_parallel=4)
    agg_obs = _test_env(vector_env, num_envs=4, num_steps=2, timeout=(0.5) * 2)
    assert len(agg_obs) == 8
    vector_env.terminate()
