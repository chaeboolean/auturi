...
from auturi.executor.shm.loop import SHMNestedLoopHandler, SHMMultiLoopHandler
...
def get_nested_loop_handler(actor_config: ActorConfig, is_discrete):
    ...
    model, policy_cls, policy_kwargs = utils.create_policy_args(is_discrete)
    loop_handler = SHMNestedLoopHandler(0, env_fns, policy_cls, policy_kwargs, \
        max_num_envs=actor_config.num_envs, max_num_policy=actor_config.num_policy, \
...
def get_multi_loop_handler(num_loops, num_collect, num_envs, is_discrete):
    env_fns = utils.create_env_fns(num_envs, is_discrete)
    model, policy_cls, policy_kwargs = utils.create_policy_args(is_discrete)
    loop_handler = SHMMultiLoopHandler(env_fns, policy_cls, policy_kwargs, \
        max_num_loop=num_loops, num_rollouts=num_collect)
...
@pytest.mark.parametrize("num_envs,is_discrete", [(1, False),  (1, True), (2, False), (2, True)])
...
@pytest.mark.parametrize("num_envs,is_discrete", [(2, False),  (2, True), (4, False), (4, True)])
...
@pytest.mark.parametrize("num_envs_per_loop,is_discrete", [(1, False), (1, True), (2, False), (3, True)])
def test_multi_loop_single_loop(num_envs_per_loop, is_discrete):
    num_collect = 3 * num_envs_per_loop
    with get_multi_loop_handler(1, num_collect, num_envs_per_loop, is_discrete) as loop_handler:
        agg_data, metric = loop_handler.run()        
