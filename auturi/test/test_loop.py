...
@pytest.mark.parametrize("is_discrete", [False])
def test_nested_loop_serial_env(is_discrete):
    config = ActorConfig(
        num_envs=4,
        num_parallel=2,
        num_policy=1,
    ...
    assert agg_data["obs"].shape == (16, 5, 2)
    utils.check_array(agg_data["action"][:, 0], [1, 2, 3, 4] * 4, keep_order=False)

    if is_discrete:
        assert agg_data["action"].shape == (4 * 4, 1)
    else:
        assert agg_data["action"].shape == (4 * 4, 3)
...
@pytest.mark.parametrize("num_envs_per_loop,is_discrete", [(1, False)])
